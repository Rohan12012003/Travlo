...
exports.handler = async function (event, context) {
  ...
  try {
    ...
    const { userId, wishlistItem } = JSON.parse(event.body); // Assuming the wishlist item is sent in the request body
    ...
    const updatedWishlist = updatedUser.wishlist; // Extract the updated wishlist array from the user object

    return {
      statusCode: 200,
      body: JSON.stringify(updatedWishlist), // Return the updated wishlist
    };
  } catch (error) {
